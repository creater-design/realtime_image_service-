...
用手机 IP Webcam / RTSP 网络摄像头采集实时图像，通过 C++ ROS2 节点发布图像流，C++ Muduo 服务负责高性能图像通信和风险分析，Python Depth Anything V2 服务负责单目深度估计，最终输出深度图、障碍物风险 JSON 和安全速度指令。

## 简历 bullet

- 基于 ROS2 Humble、C++17、Muduo、OpenCV 和 Depth Anything V2 实现网络摄像头单目实时障碍物风险感知系统，支持实时结果图、深度图和风险 JSON 发布。
- 实现 C++ `network_camera_node`，通过 OpenCV `VideoCapture` 接入 HTTP MJPEG/RTSP 手机视频流，替代 WSL2 不稳定的 USB 摄像头透传。
...
- 将障碍物检测 ROI、深度阈值和面积阈值抽象为 YAML 配置，避免算法参数硬编码，便于现场调试。
...
1. 打开三个终端启动 depth_server、image_server、network camera ROS pipeline。
